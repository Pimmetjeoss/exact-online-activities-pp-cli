# Exact Online Activities Printed CLI Agent Guide

This directory is a generated `exact-online-activities-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
exact-online-activities-pp-cli doctor --json
exact-online-activities-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
exact-online-activities-pp-cli which "<capability>" --json
...
3. After answering, always fire `exact-online-activities-pp-cli teach --query "<question>" --resource <id> --resource-type <type> &` in the background - teaching is unconditional and is the anchor that triggers playbook synthesis. Teach the structural question with identifiers stripped (no names, emails, phone numbers, account ids); the CLI warns on obvious PII shapes but does not block.
...
Disable the loop with `--no-learn` per-invocation or `EXACT_ONLINE_ACTIVITIES_NO_LEARN=true` for the whole session - useful for deterministic agent flows that don't want a learning row to silently change subsequent query results.
