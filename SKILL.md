...
- `exact-online-activities-pp-cli activities communication-notes-get` — The account that is related to the communication note; The name of the account. Official docs: https://start.
- `exact-online-activities-pp-cli activities communication-notes-post` — The account that is related to the communication note; The name of the account. Official docs: https://start.
- `exact-online-activities-pp-cli activities complaints-get` — The account that is related to the complaint; The name of the account. Official docs: https://start.exactonline.
- `exact-online-activities-pp-cli activities complaints-post` — The account that is related to the complaint; The name of the account. Official docs: https://start.exactonline.
- `exact-online-activities-pp-cli activities events-get` — The account that is related to the event; The name of the account. Official docs: https://start.exactonline.
- `exact-online-activities-pp-cli activities events-post` — The account that is related to the event; The name of the account. Official docs: https://start.exactonline.
...
- `exact-online-activities-pp-cli activities service-requests-get` — The account that is related to the service request; The name of the account. Official docs: https://start.exactonline.
...
- Use `--home <dir>` for one invocation, or set `EXACT_ONLINE_ACTIVITIES_HOME=<dir>` to relocate all four path kinds under one root.
- Use per-kind env vars only when a specific kind must diverge: `EXACT_ONLINE_ACTIVITIES_CONFIG_DIR`, `EXACT_ONLINE_ACTIVITIES_DATA_DIR`, `EXACT_ONLINE_ACTIVITIES_STATE_DIR`, `EXACT_ONLINE_ACTIVITIES_CACHE_DIR`.
- Resolution order is per-kind env var, `--home`, `EXACT_ONLINE_ACTIVITIES_HOME`, XDG (`XDG_CONFIG_HOME`, `XDG_DATA_HOME`, `XDG_STATE_HOME`, `XDG_CACHE_HOME`), then platform defaults.
- `config` contains settings like `config.toml` and profiles. `data` contains `credentials.toml`, `data.db`, cookies, and auth sidecars. `state` contains persisted queries, jobs, and `teach.log`. `cache` contains regenerable HTTP/cache files.
- Stored secrets live in `credentials.toml` under the data dir. Existing legacy `config.toml` secrets are read for compatibility and leave `config.toml` on the first auth write.
- Run `exact-online-activities-pp-cli doctor --fail-on warn` to surface path and credential-location warnings. `agent-context` exposes a schema v4 `paths` block for agents that need the resolved dirs.
...
Fleet precedence: an inherited per-kind env var overrides an explicit `--home` for that kind. Use `EXACT_ONLINE_ACTIVITIES_HOME` or per-kind vars as durable fleet levers, and use `--home` only for a single invocation. Relocation is not reversible by unsetting env vars; move files manually before clearing `EXACT_ONLINE_ACTIVITIES_HOME`, or `doctor` will not find credentials left under the former root.
...
```bash
exact-online-activities-pp-cli recall "<user's question>" --agent
```
...
```json
{
  ...
  "candidates": [
    { "id": 12, "class": "flag_alias | playbook_candidate",
      ...
      "rationale": "...",
      "next_action": ["<trial command>", "exact-online-activities-pp-cli learnings confirm 12"] }
  ],
  "playbook": {
    "query_family": "...",
    "playbook": {
...
if Playbook present:
    ...
    -> the Playbook's expected_tool_calls is a budget; if you find yourself running
       materially more, record the divergence via `exact-online-activities-pp-cli playbook amend`
       at end-of-session.
...
```bash
exact-online-activities-pp-cli teach --query "<user's question>" --resource-type <type> --resource <id1> --resource <id2>
...
exact-online-activities-pp-cli teach \
...
exact-online-activities-pp-cli teach-playbook \
  --query "<user's question>" \
  --playbook-file ~/playbooks/<shape>.json \
...
```bash
exact-online-activities-pp-cli playbook amend \
...
`exact-online-activities-pp-cli learnings stats` reports recall hit rate, teach-to-reuse, playbook resolution rate, and candidate confirm/reject counts from the local `learn_events` table. Rates are null until they have a denominator; everything stays on this machine. Use it to check whether the loop is earning its keep for this CLI.
