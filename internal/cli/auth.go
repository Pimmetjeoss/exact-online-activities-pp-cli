...
<<<<<<< HEAD
		Use:         "auth",
		Short:       "Manage authentication for Exact Online Activities",
...
func newAuthSetupCmd(_ *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: "  exact-online-activities-pp-cli auth setup\n  exact-online-activities-pp-cli auth setup --launch",
		RunE: func(cmd *cobra.Command, args []string) error {
			w := cmd.OutOrStdout()
			fmt.Fprintln(w, "See API docs for OAuth app registration: https://start.exactonline.nl/docs/HlpRestAPIResources.aspx?SourceAction=10")
			fmt.Fprintln(w, "")
			fmt.Fprintln(w, "Then run:")
<<<<<<< HEAD
			fmt.Fprintln(w, "  exact-online-activities-pp-cli login")
=======
			fmt.Fprintln(w, "  exact-online-activities-pp-cli auth login --client-id <id> --client-secret <secret>")
>>>>>>> origin/main
			if !launch {
				return nil
			}
			launchURL := "https://start.exactonline.nl/docs/HlpRestAPIResources.aspx?SourceAction=10"
...
>>>>>>> origin/main
	...
	cmd.Flags().StringVar(&clientID, "client-id", os.Getenv("EXACT_ONLINE_ACTIVITIES_CLIENT_ID"), "OAuth2 client ID")
	cmd.Flags().StringVar(&clientSecret, "client-secret", os.Getenv("EXACT_ONLINE_ACTIVITIES_CLIENT_SECRET"), "OAuth2 client secret")
...
func resolveOAuthCredentials(cmd *cobra.Command, flags *rootFlags, cfg *config.Config, clientID, clientSecret string) (string, string, error) {
	...
	if flags.noInput {
		return "", "", fmt.Errorf("OAuth2 client ID is required; pass --client-id, set EXACT_ONLINE_ACTIVITIES_CLIENT_ID, or run 'exact-online-activities-pp-cli login' without --no-input")
	}
...
<<<<<<< HEAD
			...
			if !authed {
				fmt.Fprintf(w, "  %s Not authenticated. Run 'exact-online-activities-pp-cli login' to authenticate.\n", red("FAIL"))
=======
...
<<<<<<< HEAD
					fmt.Fprintf(w, "  %s Token expired. Run 'exact-online-activities-pp-cli login' to re-authenticate.\n", red("FAIL"))
=======
