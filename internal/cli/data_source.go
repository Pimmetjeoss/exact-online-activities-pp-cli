...
>>>>>>> origin/main
		if fallbackErr != nil {
			return nil, DataProvenance{}, fmt.Errorf("API unreachable and no local data. Run 'exact-online-activities-pp-cli sync' to enable offline access.\n\nOriginal error: %w", err)
		}
...
func writeThroughCache(ctx context.Context, resourceType string, data json.RawMessage) {
	db, err := store.OpenWithContext(ctx, defaultDBPath("exact-online-activities-pp-cli"))
	if err != nil {
		return
	}
	defer db.Close()
...
func resolveLocal(ctx context.Context, flags *rootFlags, hintWriter io.Writer, resourceType string, isList bool, path string, params map[string]string, reason string) (json.RawMessage, DataProvenance, error) {
	db, err := openStoreForRead(ctx, "exact-online-activities-pp-cli")
	if err != nil {
...
func resolveLocal(ctx context.Context, resourceType string, isList bool, path string, params map[string]string, reason string) (json.RawMessage, DataProvenance, error) {
	db, err := openStoreForRead(ctx, "exact-online-activities-pp-cli")
	if err != nil {
		return nil, DataProvenance{}, fmt.Errorf("opening local database: %w\nRun 'exact-online-activities-pp-cli sync' first.", err)
	}
	if db == nil {
		return nil, DataProvenance{}, fmt.Errorf("no local data. Run 'exact-online-activities-pp-cli sync' first")
>>>>>>> origin/main
	}
	defer db.Close()
...
=======
			return nil, DataProvenance{}, fmt.Errorf("no local data for %q. Run 'exact-online-activities-pp-cli sync' first", resourceType)
>>>>>>> origin/main
		}
...
=======
			return nil, DataProvenance{}, fmt.Errorf("resource %q with ID %q not found in local store. Run 'exact-online-activities-pp-cli sync' first", resourceType, id)
>>>>>>> origin/main
		}
		return nil, DataProvenance{}, fmt.Errorf("querying local store: %w", err)
	}
