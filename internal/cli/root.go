...
type rootFlags struct {
	...
	// invocation. Mirrors the EXACT_ONLINE_ACTIVITIES_NO_LEARN env var.
	noLearn bool
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "exact-online-activities-pp-cli",
		Short: "Manage exact-online-activities resources via the exact-online-activities API",
		Long: `Manage exact-online-activities resources via the exact-online-activities API.

Add --agent to any command for JSON output + non-interactive mode.
Run 'exact-online-activities-pp-cli doctor' to verify auth and connectivity.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("exact-online-activities-pp-cli {{ .Version }}\n")
...
<<<<<<< HEAD
	...
	rootCmd.PersistentFlags().StringVar(&flags.runProfileName, "profile", "", "Apply values from a saved run profile; this does not select a client (see 'exact-online-activities-pp-cli profile list')")
...
=======
	...
	rootCmd.PersistentFlags().StringVar(&flags.dataSource, "data-source", "auto", "Data source for read commands: auto (live with local fallback), live (API only), local (synced data only)")
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'exact-online-activities-pp-cli profile list')")
>>>>>>> origin/main
...
=======
			fmt.Printf("exact-online-activities-pp-cli %s\n", version)
>>>>>>> origin/main
		},
	}
}
