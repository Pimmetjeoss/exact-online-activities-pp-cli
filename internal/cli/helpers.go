...
>>>>>>> origin/main
			...
			"\n      Run 'exact-online-activities-pp-cli doctor' to check auth status."+
...
<<<<<<< HEAD
		return authErr(fmt.Errorf("%w\nhint: check your token."+
			"\n      Run 'exact-online-activities-pp-cli auth login' to re-authenticate."+
=======
		return authErr(fmt.Errorf("%w\nhint: check your token. Set it with: exact-online-activities-pp-cli auth set-token <token>"+
			"\n      or: export EXACT_ONLINE_ACTIVITIES_OAUTH2=<your-token>"+
>>>>>>> origin/main
			"\n      See API docs: https://start.exactonline.nl/docs/HlpRestAPIResources.aspx?SourceAction=10"+
			"\n      Run 'exact-online-activities-pp-cli doctor' to check auth status.", err))
...
func nonJSONPayloadError(data json.RawMessage) error {
	...
	if len(trimmed) > 0 && trimmed[0] == '<' {
		return authErr(fmt.Errorf("not authenticated or session expired; API returned HTML instead of JSON. " + "Run 'exact-online-activities-pp-cli auth login' to re-authenticate."))
	}
	if len(trimmed) == 0 {
