...
Exit codes & warnings:
  ...
  exact-online-activities-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  exact-online-activities-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  exact-online-activities-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  exact-online-activities-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  exact-online-activities-pp-cli sync --latest-only`,
