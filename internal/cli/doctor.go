...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  exact-online-activities-pp-cli doctor
  exact-online-activities-pp-cli doctor --json
<<<<<<< HEAD
  exact-online-activities-pp-cli doctor --fail-on warn
  exact-online-activities-pp-cli doctor --fail-on stale`,
			...
			if credentialRefused {
				...
				if cfg != nil {
					...
					if !configured {
						...
						report["auth_hint"] = "exact-online-activities-pp-cli login"
						report["auth_docs_url"] = "https://start.exactonline.nl/docs/HlpRestAPIResources.aspx?SourceAction=10"
					} else {
						authConfigured = true
...
=======
			if cfg != nil {
				...
				if header == "" {
					report["auth"] = "not configured"
					report["auth_hint"] = "export EXACT_ONLINE_ACTIVITIES_OAUTH2=<your-key>"
					report["auth_docs_url"] = "https://start.exactonline.nl/docs/HlpRestAPIResources.aspx?SourceAction=10"
				} else {
					authConfigured = true
					report["auth"] = "configured"
...
>>>>>>> origin/main
			...
			if os.Getenv("EXACT_ONLINE_ACTIVITIES_OAUTH2") != "" {
				authEnvSet = append(authEnvSet, "EXACT_ONLINE_ACTIVITIES_OAUTH2")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "EXACT_ONLINE_ACTIVITIES_OAUTH2")
			}
			switch {
...
<<<<<<< HEAD
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "exact-online-activities-pp-cli", suggestion)
						} else {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'exact-online-activities-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'exact-online-activities-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'exact-online-activities-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'exact-online-activities-pp-cli sync' to refresh."
	}
	return report
}
